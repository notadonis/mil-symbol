...
- **Live search and filtering** – A search box filters symbols by name or category.  A drop‑down still lets you narrow results to a specific symbol set, but the main page now organises all symbols into collapsible sections by category.  Each section has a heading (e.g. *Air*, *Land Units*, *Control Measure*) and can be expanded or collapsed individually for easier browsing.  Use the optional **View selected** toggle to show only the cards you’ve chosen.
